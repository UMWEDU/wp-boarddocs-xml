...
namespace UMW\BoardDocs {
	...
	class Plugin {
		...
		function admin_init() {
			...
			add_settings_section( 'boarddocs-xml', __( 'BoardDocs XML' ), array(
				&$this,
				'settings_section'
			), 'general' );
			add_settings_field( 'wp-boarddocs-feed-prefix', __( 'Feed Prefix:' ), array(
				&$this,
				'settings_field'
			), 'general', 'boarddocs-xml', array( 'label_for' => 'wp-boarddocs-feed-prefix' ) );
			add_settings_field( 'wp-boarddocs-feed-default', __( 'Default Feed Type:' ), array(
				&$this,
				'settings_field'
			), 'general', 'boarddocs-xml', array( 'label_for' => 'wp-boarddocs-feed-default' ) );
		}
		...
		function settings_field_prefix( $args ) {
			...
			?>
            <input class="regular-text" type="url" name="<?php echo $args['label_for'] ?>"
                   id="<?php echo $args['label_for'] ?>" value="<?php echo $this->feed_prefix ?>"/>
		...
		protected function _determine_type( $feed ) {
			...
			$feed = strtolower( array_pop( $feed_arr ) );

			$feed_arr = explode( '-', $feed );
			return array_pop( $feed_arr );
		}
		...
		<div class="board-event-lists">';
			...
			foreach ( $xml as $cat ) {
				...
				foreach ( $cat as $event ) {
					...
					$out .= property_exists( $event, 'end' ) ? '<br/>' . ( $show_time_delimiter ? '<strong>Until:</strong> ' : '' ) . '<time class="event-date event-end" datetime="' . $event->end[0]->datetime[0] . '">' . $this->ap_date( $event->end[0]->datetime[0] ) . '</time>' : '';
		...
		function display_activemeetings( $atts = array() ) {
			...
			$show_description = array_key_exists( 'show_description', $atts ) && in_array( $atts['show_description'], array(
					'true',
					true,
					1,
					'1'
				), true );
			$show_content     = array_key_exists( 'show_content', $atts ) && in_array( $atts['show_content'], array(
					'true',
					true,
					1,
					'1'
		...
		function ap_date( $datestring ) {
			...
			if ( strstr( $datestring, 'T' ) ) {
                ...
                $arr = explode( 'T', $datestring );
				$show_time = array_pop( $arr );
